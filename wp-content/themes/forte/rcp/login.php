...
<?php if( ! is_user_logged_in() ) : ?>
		...
		<fieldset class="rcp_login_data">
			...
			<p class="rcp_text">
				<?php if (get_theme_mod('register_page_selector')) { ?>
					<a href="<?php echo get_page_link(get_theme_mod('register_page_selector')); ?>"><?php _e( 'Don&#39t have an account? Subscribe now &rarr;', 'bean' ); ?></a><br/>
				<?php } ?>

				<?php if (get_theme_mod('lostpass_page_selector')) { ?>
					<a href="<?php echo get_page_link(get_theme_mod('lostpass_page_selector')); ?>"><?php _e( 'Lost your password?', 'bean' ); ?></a><br/>
				<?php } ?>
			</p>
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			$('input#rcp_user_login').attr('placeholder', '<?php _e("Username", "bean") ?>');
			$('input#rcp_user_pass').attr('placeholder', '<?php _e("Password", "bean") ?>');
		}); 	
	</script>
...
<?php else : ?>
	...
	<div class="rcp_text logged-in">
   		<p><a href="javascript:javascript:history.go(-1)"><?php _e( 'Oh yea, I&#39;m already logged in. Take me back &rarr;', 'bean' ); ?></a></p>
	</div>
